...
// // PT. 1: HUBUNGKAN KE SUPABASE
// // Mengganti 'axios' dengan import client Supabase yang sudah Anda buat di VS Code
...
export default function Profile() {
  ...
  // STATE DATA KELAS: Menyimpan array data kelas yoga yang diambil dari tabel Supabase
  ...
  // // FUNGSI API (GET VIA SUPABASE): Mengambil seluruh data dari tabel 'YogaClasses'
  const getClassesData = async () => {
    try {
      // Mengambil data dari tabel 'YogaClasses' menggunakan query builder bawaan Supabase
      const { data, error } = await supabase
        .from('YogaClasses')
        .select('*');

      // Jika database Supabase mengembalikan error, lempar ke blok catch
      if (error) throw error;

      // Menyimpan data array hasil query ke state 'classesData'
      setClassesData(data || []);
      // Mematikan animasi loading setelah data sukses masuk
      setLoading(false);
    } catch (error) {
      // Mencetak pesan error ke konsol log jika terjadi kegagalan query
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      setLoading(true);
      // Memanggil fungsi fetch data kelas yoga dari Supabase
      getClassesData();
    }, [])
  );
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        ...
        activeOpacity={0.8}
        // // MODIFIKASI NAVIGASI: Dialihkan ke 'YogaDetail' dengan membawa data item lewat params route
